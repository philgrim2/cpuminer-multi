...
int blake2b_init_key( blake2b_state *S, size_t outlen, const void *key, size_t keylen )
{
...

  {
    ...
    blake2b_ref_update( S, block, BLAKE2B_BLOCKBYTES );
...
int blake2b( void *out, size_t outlen, const void *in, size_t inlen, const void *key, size_t keylen )
{
  ...
  blake2b_ref_update( S, ( const uint8_t * )in, inlen );
  blake2b_ref_final( S, out, outlen );
  return 0;
}
...
int main( void )
{
  ...
  for(step = 1; step < BLAKE2B_BLOCKBYTES; ++step) {
    for (i = 0; i < BLAKE2_KAT_LENGTH; ++i) {
      ...
      if ( (err = blake2b_ref_update(&S, p, mlen)) < 0) {
        goto fail;
      }
      if ( (err = blake2b_ref_final(&S, hash, BLAKE2B_OUTBYTES)) < 0) {
        goto fail;
      }
